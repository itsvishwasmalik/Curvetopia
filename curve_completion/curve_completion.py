...
def plot(paths_XYs, filename="plot.png"):
...
def complete_shape(paths_XYs):
    ...
    for XYs in paths_XYs:
        ...
        for i in range(len(XYs)):
            ...
            new_shape.append(curve1)

            if i < len(XYs) - 1:
                # Connect the current curve to the next curve
                curve2 = fit_spline(XYs[i + 1])
                connection = connect_curves(curve1, curve2)
                new_shape.append(connection)
        
        completed_shapes.append(new_shape)
    
    return completed_shapes

# Main execution
occlusion2_csv_path = '../problems/occlusion2.csv'  # Update the path accordingly
paths_XYs = read_csv(occlusion2_csv_path)
plot(paths_XYs, "original.png")  # Plot original shapes

completed_shapes = complete_shape(paths_XYs)
plot(completed_shapes, "completed.png")  # Plot completed shapes
